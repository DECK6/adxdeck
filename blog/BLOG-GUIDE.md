...
AI · AX는 특정 도구의 사용법 채널이 아니다. 업무 설계와 도입 성과, 지식 관리와 RAG, 에이전트 워크플로, 평가·거버넌스, AI 활용 학습과 제작을 함께 다룬다. Hermes는 여러 구현 사례 중 하나이며 기본 주제로 삼지 않는다. AKM, Graph Engineering, 모델·추론 시스템도 실제 업무 문제와 연결해 설명한다.

### AX 실무 가이드와 편집 우선순위

- `/blog/ax/`는 다섯 업무 관점으로 기존 AI · AX 글을 연결하는 정적 안내 페이지다. 상위 `track`은 기존 두 개를 유지한다.
- `blog/ax-guide.js`가 큐레이션 정본이며 `blog/build.js`가 `/blog/ax/index.html`을 생성한다. 생성 HTML은 직접 편집하지 않는다. 큐레이션에서 참조한 글이 없거나 AI · AX가 아니면 빌드가 실패한다.
- 새 글은 도구명보다 독자의 업무 질문을 먼저 정한다. 도입·프로세스 설계, 지식/RAG, 자동화, 평가·거버넌스, 학습·변화관리 중 기존 글이 부족한 영역을 우선한다. 새 제품 소식만으로 글을 늘리지 않는다.
- Hermes 글은 기존 검색 유입을 보존하며 유지보수한다. 특정 도구의 동작을 다른 도구에도 적용된다고 일반화하지 않는다.
- 본문이 한국어이면 `description`도 완결된 한국어 요약을 직접 작성한다. 제목은 검색 주제를 앞에 드러내되 기존 논지와 글의 목소리를 보존한다.
- 수정할 때 `date created`와 영문 alias 기반 slug는 유지하고 `date modified`만 실제 수정일로 갱신한다. 빌드는 이를 Article 수정일과 sitemap lastmod에 반영한다.
- 기본 빌드는 산출물을 영구 삭제하지 않는다. 원고 삭제·slug 변경으로 남은 경로는 리디렉션을 검토한 뒤 시스템 Trash로 옮기고 다시 빌드한다.
- 공개 원고는 AKM `80-outputs/blog` 정본과 동기화한다. 로컬 경로, 비공개 프로젝트, 운영 로그는 공개 글에 넣지 않는다.

검색 개선은 변경일 전후의 동일 페이지·검색어·국가·기기 단위로 비교한다. 클릭수와 CTR 외에 노출·순위를 함께 보고, 프로젝트 문의 성과는 검색 클릭과 분리한다. 방문 추적·외부 분석 스크립트를 임의로 추가하지 않는다.
