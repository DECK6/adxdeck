...
type: article
track: ai-ax
title: "검색된 문서는 명령이 아니다: AI 지식 파이프라인의 신뢰 경계"
description: "검색 문서와 도구 결과에 섞인 지시가 에이전트의 계획과 권한으로 바뀌지 않도록, 간접 프롬프트 주입의 위협과 데이터·제어 분리 원칙을 AKM과 사내 RAG 업무에 연결한다."
aliases:
  - "Retrieved Documents Are Data Not Instructions"
slug: retrieved-documents-are-data-not-instructions
author: 육대근
...
tags:
  ...
  - prompt-injection
  - security
thumbnail: images/retrieved-documents-are-data-not-instructions-cover.png
status: completed
publishReady: true
---
...
![아래쪽 자료 처리 경로와 위쪽의 고정된 제어 경로를 분리하고 실행 앞에 경계를 둔 추상 개념 이미지](images/retrieved-documents-are-data-not-instructions-cover.png)
...
2025년 논문 *Defeating Prompt Injections by Design*이 제안한 CaMeL은 이 부담을 시스템 구조로 옮긴다. 신뢰된 사용자 요청으로 계획을 만드는 Privileged LLM과, 외부 내용을 읽는 Quarantined LLM을 분리한다. 계획을 만드는 모델은 잠재적으로 오염된 문서 본문을 직접 보지 않는다. 외부 내용을 읽는 모델은 계획이 요청한 정보를 추출하는 역할을 맡는다.[3]
...
기록만 붙인다고 방어가 완성되는 것은 아니다. 실행기가 그 기록을 읽고 제한을 적용해야 한다. 외부 문서의 내용을 검토 없이 `approval_basis`에 복사한다면 데이터에 스스로 허가증을 쓰게 한 셈이다. 요약을 다시 검색해 쓰는 경우에도 원래의 출처와 제한이 남아 있는지 시험해야 한다.
...
[1] [OWASP, LLM01:2025 Prompt Injection](https://genai.owasp.org/llmrisk/llm01-prompt-injection/). 2025 항목, 2026-09-20 확인. RAG·미세조정의 한계와 최소 권한·외부 콘텐츠 분리·사람 승인·적대적 시험 권고.

[2] [AgentDojo: A Dynamic Environment to Evaluate Prompt Injection Attacks and Defenses for LLM Agents](https://arxiv.org/abs/2406.13352v3). 2024-11-24 개정본. 평가 환경과 정상 업무·공격 성공의 구분.

[3] [Defeating Prompt Injections by Design](https://arxiv.org/html/2503.18813v2). 2025-06-24 개정본, 특히 §2, §3.1, §5.1–5.4. 계획·자료 처리 분리, 보안 정책, 명시적 비목표.

[4] [CaMeL 공개 연구 코드와 구현상 주의사항](https://github.com/google-research/camel-prompt-injection). 2026-09-20 확인. 논문 재현용 코드의 지원·보안 한계.
