...
type: article
track: ai-ax
title: "에이전트의 보상 트랜잭션은 되감기가 아니다"
description: "여러 시스템을 바꾸는 에이전트가 중간에 실패했을 때, 보상 트랜잭션이 이미 일어난 일을 어떻게 수습하는지와 정확한 되돌리기가 불가능한 경계를 살펴본다."
aliases:
  - "Compensation Is Not Rewind for Agent Workflows"
author: 육대근
...
thumbnail: images/compensation-is-not-rewind-for-agent-workflows-cover.png
status: completed
---

# 에이전트의 보상 트랜잭션은 되감기가 아니다

![앞으로 진행한 작업 경로와 그 옆에서 반대 방향으로 작동하는 보상 경로를 추상적으로 표현한 개념 이미지](images/compensation-is-not-rewind-for-agent-workflows-cover.png)
...
작업 그래프를 만들 때 각 단계를 `재계산 가능`, `로컬 롤백 가능`, `보상 가능`, `비가역`으로 분류할 수 있다. 이 분류는 기술 속성만이 아니라 업무 규칙을 포함한다. 공개 글을 내리는 API가 있어도 독자가 이미 읽었다면 사회적 효과까지 취소되지는 않는다. 메시지를 삭제할 수 있어도 수신자의 판단은 되돌릴 수 없다. 이런 단계는 실행 전에 승인하고, 가능한 한 흐름의 뒤쪽에 배치해야 한다.
...
1. Hector Garcia-Molina and Kenneth Salem, [“Sagas”](https://dl.acm.org/doi/10.1145/38713.38742), ACM SIGMOD, 1987. Saga 패턴의 계보와 보상 트랜잭션 정의를 확인했다.
2. Microsoft Azure Architecture Center, [“Compensating Transaction pattern”](https://learn.microsoft.com/en-us/azure/architecture/patterns/compensating-transaction), 2026-04-20 갱신. 동시 변경, 업무별 보상, 보상 실패와 비가역 단계의 경계를 확인했다.
3. Temporal Documentation, [“Saga Pattern”](https://docs.temporal.io/design-patterns/saga-pattern), 2026-09-26 확인. 보상 등록 시점, 역순 실행, 멱등성과 구현별 경계를 확인했다.
4. Edward Y. Chang and Longling Geng, [“SagaLLM: Context Management, Validation, and Transaction Guarantees for Multi-Agent LLM Planning”](https://doi.org/10.14778/3750601.3750611), PVLDB 18(12), 2025, pp. 4874–4886. 상태·의존성·보상 구조와 네 가지 계획 문제의 실험 범위를 확인했다.
