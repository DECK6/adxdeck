function u(e){const a=String(e??"#5EE7F3").replace("#",""),t=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(i=>Number.parseInt(t.slice(i,i+2),16)/255)}const o={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_resolution:[e.width,e.height],u_frame:e.frame,u_t:e.t,u_fps:e.fps,u_seed:e.random("shader-seed"),u_curtains:Math.min(7,Math.max(3,Math.round(Number(e.params.curtains??5)))),u_width:Math.min(.26,Math.max(.08,Number(e.params.width??.16))),u_glow:Math.min(1,Math.max(.25,Number(e.params.glow??.68))),u_speed:Math.min(3,Math.max(1,Math.round(Number(e.params.speed??1)))),u_signal:u(e.params.signal)})}};export{o as default};
