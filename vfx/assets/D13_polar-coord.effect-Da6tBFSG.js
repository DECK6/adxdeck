function s(a){const e=String(a??"#5EE7F3").replace("#",""),u=/^[0-9a-f]{6}$/i.test(e)?e:"5EE7F3";return[0,2,4].map(t=>Number.parseInt(u.slice(t,t+2),16)/255)}const o={kind:"webgl",shader:{frag:`
...
`,uniforms:a=>({u_amount:Math.min(1,Math.max(.2,Number(a.params.amount??.9))),u_turns:Math.min(2.5,Math.max(.5,Number(a.params.turns??1))),u_radius:Math.min(1.2,Math.max(.55,Number(a.params.radius??.9))),u_speed:Math.min(2,Math.max(.5,Number(a.params.speed??1))),u_signal:s(a.params.signal)})}};export{o as default};
