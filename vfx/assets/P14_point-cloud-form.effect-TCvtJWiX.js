function r(e){const t=String(e??"#5EE7F3").replace("#",""),a=/^[0-9a-f]{6}$/i.test(t)?t:"5EE7F3";return[0,2,4].map(o=>Number.parseInt(a.slice(o,o+2),16)/255)}const n={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_density:Math.min(96,Math.max(24,Math.round(Number(e.params.density??58)))),u_pointSize:Math.min(.48,Math.max(.15,Number(e.params.pointSize??.29))),u_scatter:Math.min(1,Math.max(.1,Number(e.params.scatter??.72))),u_threshold:Math.min(.9,Math.max(.02,Number(e.params.threshold??.16))),u_signal:r(e.params.signal)})}};export{n as default};
