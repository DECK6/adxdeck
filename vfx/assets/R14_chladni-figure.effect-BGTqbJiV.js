function s(a){const o=String(a??"#5EE7F3").replace("#",""),e=/^[0-9a-f]{6}$/i.test(o)?o:"5EE7F3";return[0,2,4].map(t=>Number.parseInt(e.slice(t,t+2),16)/255)}const n={kind:"webgl",shader:{frag:`
...
`,uniforms:a=>({u_modeA:Math.min(9,Math.max(2,Math.round(Number(a.params.modeA??4)))),u_modeB:Math.min(9,Math.max(2,Math.round(Number(a.params.modeB??7)))),u_density:Math.min(.8,Math.max(.15,Number(a.params.density??.48))),u_transition:Math.min(1,Math.max(0,Number(a.params.transition??.72))),u_signal:s(a.params.signal)})}};export{n as default};
