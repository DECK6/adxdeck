function t(a){const e=String(a??"#5EE7F3").replace("#",""),o=/^[0-9a-f]{6}$/i.test(e)?e:"5EE7F3";return[0,2,4].map(l=>Number.parseInt(o.slice(l,l+2),16)/255)}const s={kind:"webgl",shader:{frag:`
...
`,uniforms:a=>({u_amount:Math.min(.32,Math.max(0,Number(a.params.amount??.19))),u_scale:Math.min(18,Math.max(4,Number(a.params.scale??10))),u_viscosity:Math.min(1,Math.max(.25,Number(a.params.viscosity??.68))),u_speed:Math.min(3,Math.max(1,Math.round(Number(a.params.speed??1)))),u_signal:t(a.params.signal)})}};export{s as default};
