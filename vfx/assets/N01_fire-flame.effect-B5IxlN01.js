function s(e){const a=String(e??"#5EE7F3").replace("#",""),t=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(o=>Number.parseInt(t.slice(o,o+2),16)/255)}const u={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_intensity:Math.min(1.4,Math.max(.35,Number(e.params.intensity??.9))),u_scale:Math.min(7,Math.max(2,Number(e.params.scale??4.2))),u_turbulence:Math.min(1.2,Math.max(.2,Number(e.params.turbulence??.72))),u_speed:Math.min(3,Math.max(1,Math.round(Number(e.params.speed??1)))),u_signal:s(e.params.signal)})}};export{u as default};
