function t(e){const a=String(e??"#5EE7F3").replace("#",""),r=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(o=>Number.parseInt(r.slice(o,o+2),16)/255)}const l={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_scale:Math.min(1.35,Math.max(.7,Number(e.params.scale??1))),u_interference:Math.min(2,Math.max(.3,Number(e.params.interference??1.15))),u_wobble:Math.min(1,Math.max(0,Number(e.params.wobble??.58))),u_refraction:Math.min(.08,Math.max(0,Number(e.params.refraction??.026))),u_signal:t(e.params.signal)})}};export{l as default};
