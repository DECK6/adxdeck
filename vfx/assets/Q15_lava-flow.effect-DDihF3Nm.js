function l(o){const e=String(o??"#5EE7F3").replace("#",""),t=/^[0-9a-f]{6}$/i.test(e)?e:"5EE7F3";return[0,2,4].map(a=>Number.parseInt(t.slice(a,a+2),16)/255)}const s={kind:"webgl",shader:{frag:`
...
`,uniforms:o=>({u_viscosity:Math.min(1,Math.max(.2,Number(o.params.viscosity??.76))),u_heat:Math.min(1.3,Math.max(.35,Number(o.params.heat??.9))),u_crust:Math.min(.9,Math.max(.1,Number(o.params.crust??.56))),u_scale:Math.min(7,Math.max(2,Number(o.params.scale??4.3))),u_signal:l(o.params.signal)})}};export{s as default};
