function n(e){const a=String(e??"#5EE7F3").replace("#",""),o=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(t=>Number.parseInt(o.slice(t,t+2),16)/255)}const i={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_maxLevels:Math.min(10,Math.max(4,Math.round(Number(e.params.maxLevels??10)))),u_contrast:Math.min(2,Math.max(.5,Number(e.params.contrast??1.2))),u_motionSpeed:Math.min(3,Math.max(1,Math.round(Number(e.params.motionSpeed??1)))),u_motionIntensity:Math.min(1,Math.max(.2,Number(e.params.motionIntensity??.78))),u_signal:n(e.params.signal)})}};export{i as default};
