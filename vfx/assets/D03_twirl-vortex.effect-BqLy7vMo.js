function t(a){const e=String(a??"#5EE7F3").replace("#",""),l=/^[0-9a-f]{6}$/i.test(e)?e:"5EE7F3";return[0,2,4].map(s=>Number.parseInt(l.slice(s,s+2),16)/255)}const o={kind:"webgl",shader:{frag:`
...
`,uniforms:a=>({u_twist:Math.min(7,Math.max(.5,Number(a.params.twist??4.2))),u_radius:Math.min(.85,Math.max(.2,Number(a.params.radius??.58))),u_speed:Math.min(3,Math.max(1,Math.round(Number(a.params.speed??1)))),u_signal:t(a.params.signal)})}};export{o as default};
