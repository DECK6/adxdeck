function n(a){const e=String(a??"#5EE7F3").replace("#",""),t=/^[0-9a-f]{6}$/i.test(e)?e:"5EE7F3";return[0,2,4].map(o=>Number.parseInt(t.slice(o,o+2),16)/255)}const i={kind:"webgl",shader:{frag:`
...
`,uniforms:a=>({u_dotSize:Math.min(24,Math.max(4,Number(a.params.dotSize??10))),u_contrast:Math.min(2,Math.max(.5,Number(a.params.contrast??1.25))),u_motionSpeed:Math.min(3,Math.max(1,Math.round(Number(a.params.motionSpeed??1)))),u_motionIntensity:Math.min(1,Math.max(.15,Number(a.params.motionIntensity??.72))),u_signal:n(a.params.signal)})}};export{i as default};
