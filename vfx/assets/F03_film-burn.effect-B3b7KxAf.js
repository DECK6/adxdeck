function r(e){const o=String(e??"#5EE7F3").replace("#",""),a=/^[0-9a-f]{6}$/i.test(o)?o:"5EE7F3";return[0,2,4].map(t=>Number.parseInt(a.slice(t,t+2),16)/255)}function s(e){return e==="right"?1:e==="center"?2:0}const i={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_resolution:[e.width,e.height],u_frame:e.frame,u_t:Math.min(1,Math.max(0,e.t)),u_fps:e.fps,u_seed:e.random("film-burn:seed")*997,u_intensity:Math.min(1.4,Math.max(.3,Number(e.params.intensity??.92))),u_turbulence:Math.min(6,Math.max(1,Number(e.params.turbulence??3.4))),u_softness:Math.min(.2,Math.max(.02,Number(e.params.softness??.08))),u_direction:s(e.params.direction),u_signal:r(e.params.signal)})}};export{i as default};
