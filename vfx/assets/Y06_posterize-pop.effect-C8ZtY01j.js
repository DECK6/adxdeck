function o(e){const a=String(e??"#5EE7F3").replace("#",""),l=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(t=>Number.parseInt(l.slice(t,t+2),16)/255)}const n={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_levels:Math.min(5,Math.max(2,Math.round(Number(e.params.levels??3)))),u_offset:Math.min(14,Math.max(0,Number(e.params.offset??6))),u_contrast:Math.min(2,Math.max(.7,Number(e.params.contrast??1.35))),u_signal:o(e.params.signal),u_fps:e.fps,u_seed:e.random("shader-seed")})}};export{n as default};
