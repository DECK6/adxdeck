function c(e){const o=String(e??"#5EE7F3").replace("#",""),u=/^[0-9a-f]{6}$/i.test(o)?o:"5EE7F3";return[0,2,4].map(a=>Number.parseInt(u.slice(a,a+2),16)/255)}const r={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_coverage:Math.min(.85,Math.max(.2,Number(e.params.coverage??.58))),u_density:Math.min(1,Math.max(.3,Number(e.params.density??.76))),u_scale:Math.min(6,Math.max(1.5,Number(e.params.scale??3.2))),u_speed:Math.min(2,Math.max(.5,Number(e.params.speed??1))),u_signal:c(e.params.signal)})}};export{r as default};
