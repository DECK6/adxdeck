function s(e){const a=String(e??"#5EE7F3").replace("#",""),i=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(o=>Number.parseInt(i.slice(o,o+2),16)/255)}const t={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_density:Math.min(1,Math.max(.25,Number(e.params.density??.72))),u_layers:Math.min(5,Math.max(2,Math.round(Number(e.params.layers??4)))),u_speed:Math.min(3,Math.max(1,Math.round(Number(e.params.speed??1)))),u_signal:s(e.params.signal)})}};export{t as default};
