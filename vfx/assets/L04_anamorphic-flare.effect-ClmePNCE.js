function s(e){const t=String(e??"#5EE7F3").replace("#",""),o=/^[0-9a-f]{6}$/i.test(t)?t:"5EE7F3";return[0,2,4].map(a=>Number.parseInt(o.slice(a,a+2),16)/255)}const r={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_intensity:Math.min(2,Math.max(0,Number(e.params.intensity??1.15))),u_width:Math.min(1.4,Math.max(.2,Number(e.params.width??.82))),u_streak:Math.min(.12,Math.max(.01,Number(e.params.streak??.045))),u_speed:Math.min(3,Math.max(1,Math.round(Number(e.params.speed??1)))),u_signal:s(e.params.signal)})}};export{r as default};
