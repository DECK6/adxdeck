...
`,uniforms:e=>({u_bleed:Math.min(1.5,Math.max(.2,Number(e.params.bleed??.85))),u_grain:Math.min(1,Math.max(.1,Number(e.params.grain??.58))),u_pigment:Math.min(1.3,Math.max(.35,Number(e.params.pigment??.9))),u_signal:u(e.params.signal),u_fps:e.fps,u_seed:e.random("shader-seed")})}};export{t as default};
