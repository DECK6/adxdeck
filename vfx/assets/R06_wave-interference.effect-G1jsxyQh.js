function o(e){const s=String(e??"#5EE7F3").replace("#",""),u=/^[0-9a-f]{6}$/i.test(s)?s:"5EE7F3";return[0,2,4].map(a=>Number.parseInt(u.slice(a,a+2),16)/255)}const n={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_frequency:Math.min(32,Math.max(8,Number(e.params.frequency??19))),u_sources:Math.min(4,Math.max(2,Math.round(Number(e.params.sources??3)))),u_contrast:Math.min(1.5,Math.max(.4,Number(e.params.contrast??1))),u_speed:Math.min(2,Math.max(.5,Number(e.params.speed??1))),u_signal:o(e.params.signal)})}};export{n as default};
