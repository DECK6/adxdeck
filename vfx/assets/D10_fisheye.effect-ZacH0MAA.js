function t(e){const a=String(e??"#5EE7F3").replace("#",""),s=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(r=>Number.parseInt(s.slice(r,r+2),16)/255)}const u={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_strength:Math.min(1.2,Math.max(.1,Number(e.params.strength??.72))),u_radius:Math.min(.72,Math.max(.3,Number(e.params.radius??.52))),u_compression:Math.min(.8,Math.max(.1,Number(e.params.compression??.48))),u_speed:Math.min(2,Math.max(.5,Number(e.params.speed??1))),u_signal:t(e.params.signal)})}};export{u as default};
