function i(t){const a=String(t??"#5EE7F3").replace("#",""),o=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(e=>Number.parseInt(o.slice(e,e+2),16)/255)}const u={kind:"webgl",shader:{frag:`
...
`,uniforms:t=>({u_intensity:Math.min(1,Math.max(.2,Number(t.params.intensity??.78))),u_curtains:Math.min(7,Math.max(2,Math.round(Number(t.params.curtains??4)))),u_bend:Math.min(1.4,Math.max(.2,Number(t.params.bend??.82))),u_speed:Math.min(2,Math.max(.5,Number(t.params.speed??1))),u_signal:i(t.params.signal)})}};export{u as default};
