function r(e){const a=String(e??"#5EE7F3").replace("#",""),t=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(i=>Number.parseInt(t.slice(i,i+2),16)/255)}const s={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_bitDepth:Math.min(8,Math.max(2,Math.round(Number(e.params.bitDepth??4)))),u_blockSize:Math.min(20,Math.max(2,Math.round(Number(e.params.blockSize??8)))),u_intensity:Math.min(1,Math.max(0,Number(e.params.intensity??.86))),u_speed:Math.min(3,Math.max(1,Math.round(Number(e.params.speed??2)))),u_signal:r(e.params.signal)})}};export{s as default};
