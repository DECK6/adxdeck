...
var O24_ring_focus_effect_exports = {};
__export(O24_ring_focus_effect_exports, {
  default: () => O24_ring_focus_effect_default
});
module.exports = __toCommonJS(O24_ring_focus_effect_exports);
const TAU = Math.PI * 2;
const LEAD = 0.05;
const SPAN = 0.72;
const STAGGER = 0.15;
...
const kernel = {
  ...
  render: (ctx) => {
    ...
    const hit = lanes.reduce(
      (sum, lane) => lane.since >= 0 ? sum + pulse * Math.exp(-16 * lane.since) * Math.cos(TAU * 1.9 * lane.since) : sum,
      0
    );
      ...
      return /* @__PURE__ */ h(
        "div",
        {
          ...
          style: {
            ...
            marginTop: -diameter / 2,
            borderRadius: "50%",
            border: \`\${Math.max(1, unit * 5e-3 * (1 - age))}px solid \${signal}\`,
...
var O24_ring_focus_effect_default = kernel;
`;export{n as default};
