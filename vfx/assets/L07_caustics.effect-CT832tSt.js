function s(e){const a=String(e??"#5EE7F3").replace("#",""),t=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(o=>Number.parseInt(t.slice(o,o+2),16)/255)}const c={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_scale:Math.min(14,Math.max(3,Number(e.params.scale??7.5))),u_intensity:Math.min(1.5,Math.max(0,Number(e.params.intensity??.9))),u_refraction:Math.min(.08,Math.max(0,Number(e.params.refraction??.028))),u_speed:Math.min(2,Math.max(.5,Number(e.params.speed??1))),u_signal:s(e.params.signal)})}};export{c as default};
