function r(e){const t=String(e??"#5EE7F3").replace("#",""),o=/^[0-9a-f]{6}$/i.test(t)?t:"5EE7F3";return[0,2,4].map(i=>Number.parseInt(o.slice(i,i+2),16)/255)}const n={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_pixelSize:Math.min(10,Math.max(1,Math.round(Number(e.params.pixelSize??3)))),u_levels:Math.min(8,Math.max(2,Math.round(Number(e.params.levels??4)))),u_motionSpeed:Math.min(3,Math.max(1,Math.round(Number(e.params.motionSpeed??1)))),u_motionIntensity:Math.min(1,Math.max(.2,Number(e.params.motionIntensity??.85))),u_signal:r(e.params.signal)})}};export{n as default};
