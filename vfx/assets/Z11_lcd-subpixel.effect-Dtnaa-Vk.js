function u(e){const a=String(e??"#5EE7F3").replace("#",""),o=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(r=>Number.parseInt(o.slice(r,r+2),16)/255)}const t={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_fps:e.fps,u_seed:e.random("lcd-seed"),u_scale:Math.min(96,Math.max(18,Math.round(Number(e.params.scale??42)))),u_gap:Math.min(.22,Math.max(.02,Number(e.params.gap??.08))),u_contrast:Math.min(1.8,Math.max(.7,Number(e.params.contrast??1.22))),u_signal:u(e.params.signal)})}};export{t as default};
