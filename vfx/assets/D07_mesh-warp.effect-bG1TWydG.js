function o(e){const a=String(e??"#5EE7F3").replace("#",""),s=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(t=>Number.parseInt(s.slice(t,t+2),16)/255)}const l={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_strength:Math.min(.18,Math.max(.01,Number(e.params.strength??.085))),u_columns:Math.min(9,Math.max(3,Math.round(Number(e.params.columns??5)))),u_fluidity:Math.min(1.4,Math.max(.2,Number(e.params.fluidity??.8))),u_speed:Math.min(2,Math.max(.5,Number(e.params.speed??1))),u_signal:o(e.params.signal)})}};export{l as default};
