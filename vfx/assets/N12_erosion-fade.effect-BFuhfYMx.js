function t(e){const o=String(e??"#5EE7F3").replace("#",""),s=/^[0-9a-f]{6}$/i.test(o)?o:"5EE7F3";return[0,2,4].map(a=>Number.parseInt(s.slice(a,a+2),16)/255)}const r={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_scale:Math.min(10,Math.max(2,Number(e.params.scale??5.5))),u_edgeWidth:Math.min(.18,Math.max(.02,Number(e.params.edgeWidth??.075))),u_roughness:Math.min(1.2,Math.max(.2,Number(e.params.roughness??.78))),u_speed:Math.min(3,Math.max(1,Math.round(Number(e.params.speed??1)))),u_signal:t(e.params.signal)})}};export{r as default};
