function l(e){const a=String(e??"#5EE7F3").replace("#",""),u=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(o=>Number.parseInt(u.slice(o,o+2),16)/255)}const s={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_spread:Math.min(1,Math.max(.2,Number(e.params.spread??.72))),u_turbulence:Math.min(1.4,Math.max(.2,Number(e.params.turbulence??.88))),u_detail:Math.min(6,Math.max(2,Number(e.params.detail??4.1))),u_speed:Math.min(2,Math.max(.5,Number(e.params.speed??1))),u_signal:l(e.params.signal)})}};export{s as default};
