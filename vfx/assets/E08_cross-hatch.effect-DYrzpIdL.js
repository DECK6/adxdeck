function n(a){const e=String(a??"#5EE7F3").replace("#",""),i=/^[0-9a-f]{6}$/i.test(e)?e:"5EE7F3";return[0,2,4].map(t=>Number.parseInt(i.slice(t,t+2),16)/255)}const s={kind:"webgl",shader:{frag:`
...
`,uniforms:a=>({u_spacing:Math.min(22,Math.max(5,Number(a.params.spacing??11))),u_lineWidth:Math.min(3.5,Math.max(.6,Number(a.params.lineWidth??1.4))),u_contrast:Math.min(2,Math.max(.6,Number(a.params.contrast??1.25))),u_speed:Math.min(3,Math.max(1,Math.round(Number(a.params.speed??1)))),u_signal:n(a.params.signal)})}};export{s as default};
