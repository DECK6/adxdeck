function l(a){const e=String(a??"#5EE7F3").replace("#",""),u=/^[0-9a-f]{6}$/i.test(e)?e:"5EE7F3";return[0,2,4].map(r=>Number.parseInt(u.slice(r,r+2),16)/255)}const s={kind:"webgl",shader:{frag:`
...
`,uniforms:a=>({u_fps:a.fps,u_seed:a.random("shader-seed")*4096,u_cells:Math.min(28,Math.max(8,Math.round(Number(a.params.cells??16)))),u_warp:Math.min(.6,Math.max(.05,Number(a.params.warp??.34))),u_pulse:Math.min(.5,Math.max(.05,Number(a.params.pulse??.26))),u_cycles:Math.min(3,Math.max(1,Math.round(Number(a.params.cycles??1)))),u_signal:l(a.params.signal)})}};export{s as default};
