function n(e){const a=String(e??"#5EE7F3").replace("#",""),u=/^[0-9a-f]{6}$/i.test(a)?a:"5EE7F3";return[0,2,4].map(r=>Number.parseInt(u.slice(r,r+2),16)/255)}const t={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_amount:Math.min(.18,Math.max(0,Number(e.params.amount??.075))),u_angle:Math.min(45,Math.max(-45,Number(e.params.angle??18)))*(Math.PI/180),u_speed:Math.min(3,Math.max(.5,Number(e.params.speed??1))),u_signal:n(e.params.signal)})}};export{t as default};
