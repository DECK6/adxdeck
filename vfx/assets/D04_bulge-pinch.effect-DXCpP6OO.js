function u(a){const e=String(a??"#5EE7F3").replace("#",""),t=/^[0-9a-f]{6}$/i.test(e)?e:"5EE7F3";return[0,2,4].map(s=>Number.parseInt(t.slice(s,s+2),16)/255)}const r={kind:"webgl",shader:{frag:`
...
`,uniforms:a=>({u_amount:Math.min(1,Math.max(-1,Number(a.params.amount??.72))),u_radius:Math.min(.72,Math.max(.18,Number(a.params.radius??.42))),u_travel:Math.min(.28,Math.max(0,Number(a.params.travel??.16))),u_speed:Math.min(3,Math.max(1,Math.round(Number(a.params.speed??1)))),u_signal:u(a.params.signal)})}};export{r as default};
