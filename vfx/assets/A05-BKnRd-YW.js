...
var A05_oscilloscope_effect_exports = {};
__export(A05_oscilloscope_effect_exports, {
  default: () => A05_oscilloscope_effect_default
});
module.exports = __toCommonJS(A05_oscilloscope_effect_exports);
const TAU = Math.PI * 2;
...
const kernel = {
  ...
  draw: (g, ctx) => {
    ...
    const bands = Array.from(
      { length: 8 },
      (_, index) => clamp(Number(sourceBands[index] ?? 0), 0, 1)
    );
    ...
    g.stroke();
    g.restore();
    const pointAt = (index, phaseOffset) => {
    ...
    const strokeWave = (phaseOffset, alpha, lineWidth) => {
      g.globalAlpha = alpha;
...
var A05_oscilloscope_effect_default = kernel;
`;export{n as default};
