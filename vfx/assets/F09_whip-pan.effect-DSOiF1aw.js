function c(e){const t=String(e??"#5EE7F3").replace("#",""),u=/^[0-9a-f]{6}$/i.test(t)?t:"5EE7F3";return[0,2,4].map(r=>Number.parseInt(u.slice(r,r+2),16)/255)}const a={kind:"webgl",shader:{frag:`
...
`,uniforms:e=>({u_direction:String(e.params.direction??"left")==="right"?1:-1,u_strength:Math.min(.28,Math.max(.02,Number(e.params.strength??.16))),u_shutter:Math.min(2.5,Math.max(.4,Number(e.params.shutter??1.1))),u_chroma:Math.min(1,Math.max(0,Number(e.params.chroma??.38))),u_signal:c(e.params.signal)})}};export{a as default};
