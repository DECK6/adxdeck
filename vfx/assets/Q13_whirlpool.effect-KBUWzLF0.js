function r(a){const s=String(a??"#5EE7F3").replace("#",""),e=/^[0-9a-f]{6}$/i.test(s)?s:"5EE7F3";return[0,2,4].map(u=>Number.parseInt(e.slice(u,u+2),16)/255)}const o={kind:"webgl",shader:{frag:`
...
`,uniforms:a=>{const s=String(a.params.spin??"2");return{u_pull:Math.min(1,Math.max(.1,Number(a.params.pull??.64))),u_spin:s==="1"?1:s==="3"?3:2,u_radius:Math.min(.75,Math.max(.25,Number(a.params.radius??.52))),u_depth:Math.min(1,Math.max(.2,Number(a.params.depth??.7))),u_signal:r(a.params.signal)}}}};export{o as default};
